...
const CountryList = () => {
  ...
  return (
    ...
    <div className="country-sectionList min-h-screen">
      <div className="lg:flex justify-center items-center gap-16 w-3/4 mx-auto pt-12 md:pt-20">
        <div className="w-full ">
          <h1 className="text-xl lg:text-1xl xl:text-3xl text-center color"style={{ color: 'black',fontFamily: 'Lora', fontWeight: 'bold' }}>
          ...
          <Dialog open={open} onClose={handleClose}>
            ...
            <DialogContent dividers>
              <select
                className="w-full p-2 h-12 border  mt-4"
                value={name}
          ...
          {isLoading || submitLoading ? (
            ...
            <div className="mt-8 grid grid-cols-4 gap-6 md:w-4/5 mx-auto ">
  {countries.map((country) => (
    <Link
      to={`/country/${country.countryId}`}
      key={country.countryId}
      className="country-List mt-4 w-full border hover:bg-teal-100 flex flex-col items-center"
    >
      <img
        src={country.flag}
        alt={country.language}
        className="h-20 w-32 mt-3 mb-3 border-4 border-mix-silver-white rounded-md bg-transparent"
      />
      <p className="text-center font-serif text-gray-500">{country.language}</p>
    </Link>
  ))}
</div>
