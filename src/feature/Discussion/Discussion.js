...
const Discussion = () => {
  ...
  return (
    <div className="w-4/4 mx-auto mt-8 md:mt-8 my-4 min-h-screen">
      <h1 className="text-xl lg:text-3xl xl:text-4xl font-serif text-center">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="w-2/4 md:w-2/4 lg:w-1/4 md:pl-24 flex ">
            <Button variant="contained" onClick={handleClickOpen}
             style={{ backgroundColor: '#17B9A9', color: 'white' }}>
              Create Discussion
            </Button>
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-4 md:w-3/4 mt-16 ">
            {discussion.map((dsc) => (
              ...
              >
                ...
                <h1 className="text-sm text-start font-semibold text-gray-400 capitalize mt-2" style={{ display: "flex", justifyContent: "space-between" }}>
  <span className="text-gray-900">{newFormatDate(dsc.createdAt)}</span>
  <Link to={`/discussion/${dsc.discussionId}`} className="inline-block px-4 py-2 bg-teal-500 text-white rounded-md hover:bg-teal-600">
    Answers
  </Link>
</h1>
              </div>
